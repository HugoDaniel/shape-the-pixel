 ...
 * 
 * It assumes that the file path can be found on `Deno.cwd`. (see `cli.ts`)
 * 
...
async function runDeno(args: Map<string, string>, options?: DenoTagOptions) {
  ...
  const runOptions = options?.runOptions || {
    cmd: ["deno", "run"],
    stdout: "piped",
  }; // ^ by default call "deno run" with the stdout piped to a Uint8Array
  ...
  switch (action) {
    case "run":
      // Append the [file] to the command line array to be run
      runOptions.cmd.push(file);
      try {
...
function parseDenoTagArgs(line: string): DenoTagAttributes[] {
  ...
  for (let i = 1; i < denoTags.length; i++) {
    const attributes = denoTags[i].slice(0, denoTags[i].indexOf(">"));
    const keyValues: [string, string][] =
      // tag attributes are separated by white space
      attributes.split(" ")
        // remove those that don't have =
        .filter((s) => s.indexOf("=") >= 0)
        // break the attribute `key="value"` into the pair [key, value]
        .map((attribute) => {
          const [key, value] = attribute.split("=").splice(0, 2);
          // remove the inside quotes "" from "value"
...
async function executeDenoTags(
  ...
  // Loop through each tag found and its attributes and line info
