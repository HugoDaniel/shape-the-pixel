...
import type { ModulesFunctions } from "../../../dependencies/shader_canvas/core/common/program_class.ts";
import {
  transformPoint,
  identityMatrix,
  translate,
  scale,
  multiply,
} from "https://deno.land/x/simple_shape_math@v1.0.0/matrix.ts";
...
export class PanZoomState {
   ...
   * [panX, panY, zoom]
   **/
  originAt: [number, number, number];
  private _isUpdating = false;
  private _timeoutId: number | null = null;

  constructor(zoom: number, zoomT: Float32Array) {
    this.matrix = zoomT;
   ...
   **/
  static isPanZoomState(s: unknown): s is PanZoomState {
...
export class PanZoom2D {
  ...
  startEventListeners() {
    ...
    addEventListener(
      "gesturestart",
      this.gestureStartHandler as EventListenerOrEventListenerObject
    );
    addEventListener(
      "gesturechange",
      this.gestureChangeHandler as EventListenerOrEventListenerObject
    );
   ...
   */
  initialize({ dpr = window.devicePixelRatio }: { dpr: number }) {
    this.reset(dpr);
    const module: () => ModulesFunctions = () => {
      ...
      const totalPanZoom: [number, number, number] = [0, 0, this.zoom];
      ...
      const translateToCenter: [number, number, number] = [0, 0, 0];
      const translateFromCenter: [number, number, number] = [0, 0, 0];
      const scaleDelta: [number, number, number] = [0, 0, 0];
   ...
   * this function.
   */
  gestureStartHandler = (e: GestureEvent) => {
    e.preventDefault();
   ...
   * `gestureStartHandler`.
   */
  gestureChangeHandler = (e: GestureEvent) => {
    e.preventDefault();
   ...
   */
  gestureEndHandler = (e: Event) => {
    e.preventDefault();
  };
}

/**
 * TypeScript has no GestureEvent as of this writing. This is a simple
 * polyfill.
 */
