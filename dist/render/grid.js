import {
	...
	DrawCalls,
	ReadPixels,
} from "../dependencies/shader_canvas.js"
import { PanZoomState } from "./pan_zoom_2d.js"
import { Matrix, Containers } from "../dependencies/bundle.js"
const { ortho } = Matrix
...
*/
	...
	fillInstanceScene(
		...
		for (let i = 0; i < this.instances; i++) {
			...
			this.instanceScene[3 * i + 2] =
				gridFillIds.getXY(sceneX, sceneY) || 0
		}
	...
	initializeBuffers(gridFillIds) {
		ShaderCanvas.initializeBuffers((gl, buffers) => {
			...
			if (!PanZoomState.isPanZoomState(state)) {
				console.warn(
					"Unable to get the state for the 'pan-zoom-2d' module"
				)
				return
			}
			...
			return () => {
				if (
					...
					console.warn(
						'Invalid "pan-zoom-2d" module state in grid buffers'
					)
					return
				}
